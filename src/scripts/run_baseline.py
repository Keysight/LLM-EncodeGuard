...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run baseline LLM security tests")
    parser.add_argument(
        "--provider",
        type=str,
...
def run_test_for_model(args, model_name, logger, endpoint_from_config=None, timestamp=None):
    """
    Run baseline tests for a specific model.
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"Starting baseline testing for model: {model_name}")
    ...
    logger.info(f"Testing {len(prompt_indices)} prompts")
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as f:
        ...
        for idx in prompt_indices:
            ...
            system_prompt, user_prompt = all_pairs[idx]

            logger.info(f"\n{'='*60}")
            logger.info(f"Testing prompt {idx + 1}/{len(all_pairs)}")
            ...
            try:
                ...
                if args.delay > 0 and idx < len(prompt_indices) - 1:
